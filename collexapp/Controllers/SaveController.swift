        ...
        db.collection("saves")
            ...
            if let err = err {
                ...
                for document in querySnapshot!.documents {
                    ...
                    let save = Mapper<SaveProduct>().map(JSONObject: document.data())!
                    let saveJson = Mapper().toJSONString(save, prettyPrint: true)!
                    print("Current Save: \(saveJson)")
                    self.db.collection("products").document(save.productId).getDocument { (document, error) in
                        if let document = document, document.exists {
                            let currentProduct = Mapper<Product>().map(JSONObject: document.data())!
                            let productJson = Mapper().toJSONString(currentProduct, prettyPrint: true)!
                            print("Current Product: \(productJson)")
                            self.db.collection("users").document(currentProduct.sellerId).getDocument { (document, error) in
                                if let document = document, document.exists {
                                    ...
                                    let userJson = Mapper().toJSONString(currentUser, prettyPrint: true)!
                                    print("Current User: \(userJson)")
